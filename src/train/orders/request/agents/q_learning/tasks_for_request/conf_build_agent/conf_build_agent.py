...
class ConfBuildAgentTask:
    ...
    def __init__(self, cache_id_train, cache_id_validation=None):
    ...
    def _apply_sub_task(self, key, value):
        return value if 'interface_epoch' not in key else self._conf_interface(value, 'validation' in key)

    
    def _conf_interface(self, dict_interface, val_data):
        return self._make_interface_task(self._id_cache_validation if val_data else self._id_cache_train,
                                         dict_interface)
