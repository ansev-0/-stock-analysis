...
class TransitionsFractions(ControllerTransitions):
    ...
    def _sell_transition(self, return_future_parameters, index, inventory):
        ...
        if inventory_not_empty:
            ...
            profit = self._total_profit(index, len(inventory))
            ...
            # if mean of inventory isn't greater than current value, incr reward
                ...
                return reward, profit, self._max_buy_next(index), income, inventory, inventory_not_empty
        ...
        if return_future_parameters:
            ...
            reward = self._sell_pred_rewards[index]  * self._gamma_future
            reward += profit * self._gamma_profit
            return reward, profit, self._max_buy_next(index), 0, inventory, inventory_not_empty

        return profit, self._money, 0, inventory, inventory_not_empty
    ...
    def _sell_perc_transition(self, return_future_parameters, index, inventory, perc):
        ...
        if inventory_not_empty:
            ...
            if return_future_parameters:
                ...
                reward = self._update_reward_size_transaction(reward, n)
                reward += profit * self._gamma_profit

                return reward, profit, self._max_buy_next(index), \
    ...
    def _sell_all_transition(self, return_future_parameters, index, inventory):
        ...
        if inventory_not_empty:
            ...
            if return_future_parameters:
                ...
                reward = self._update_reward_size_transaction(reward, len_inventory)
                reward += profit * self._gamma_profit
        ...
        if return_future_parameters:
            ...
            reward = self._sell_pred_rewards[index] * self._gamma_future + profit * self._gamma_profit
            return reward, profit, self._max_buy_next(index), 0, inventory, inventory_not_empty
            ...
            if return_future_parameters:
             ...
             # total reward
                reward = self._buy_pred_rewards[index] * self._gamma_future + reward_inventory \
                    + profit * self._gamma_profit
                return reward, profit, self._max_buy_next(index), income, inventory, enough_money

            return profit, self._money, income, inventory, enough_money

        else:

            profit = self._total_profit(index, len_inventory)

        if return_future_parameters:

            reward = self._buy_pred_rewards[index] * self._gamma_future + profit * self._gamma_profit
            return reward, profit, self._max_buy_next(index), 0, inventory, enough_money

        return profit, self._money, 0, inventory, enough_money
    ...
    def _buy_perc_transition(self,return_future_parameters, index, inventory, perc):
        ...
        if enough_money:
            ...
            if return_future_parameters:
                ...
                reward = self._update_reward_size_transaction(reward, n) + profit * self._gamma_profit
                return reward, profit, \
    ...
    def _not_actions_transition(self, return_future_parameters, index, inventory):
        ...
        if return_future_parameters:
            reward = self._not_actions_pred_rewards[index] + profit * self._gamma_profit
            return reward, profit, self._max_buy_next(index), 0, inventory, True
        return profit, self._money, 0, inventory, True
    ...
    def _total_profit(self, index, size_inventory):
        return (self._positions[index] - self._commision) * size_inventory + self._money - self.init_money
