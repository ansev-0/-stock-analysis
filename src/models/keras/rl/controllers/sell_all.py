...
class TransitionsSellAll(ControllerTransitions):
    ...
    def _transition_done(self, index, inventory):
        income = (self._positions[index] - self.commision) * len(inventory) 
    ...
    def _sell_all_transition(self, index, inventory):
        ...
        if inventory_not_empty:
            ...
            # based on inventory
            gross_income = self._positions[index] * len_inventory
            income = gross_income - (self._commision * len_inventory)
            self._money += income
    ...
    def _not_actions_transition(self, index, inventory):
        ...
        reward = unit_reward_future
        return reward, self._money, 0, inventory
