...
class SplitIO:
    ...
    def dict_array_from_array(self, array, keys=None):
        return dict(zip(self.__get_valid_keys(keys), self.tuple_array_from_array(array)))                                                 
    ...
    def __create_tuple_from_frame(self, dataframe, transform_function):
        return tuple(transform_function(group) 
                     for _, group in self.__groups(dataframe, lambda arr: arr))
    ...
    def __get_by_array(self, dataframe):
        ...
        return np.arange(n_cols) < n_cols-self.output_delays
