...
class CheckErrorsFromAlphaVantage(CheckToDataBaseIntraday):
    ...
    def check_frecuency_in_api(self, frecuency):
        if not self.__frecuency_supported(frecuency=frecuency, list_frecuencies=self.frecuencies_intraday_in_api):
            raise ToDataBaseAlphaVantageError(f'Frequency not supported, frequencies supported are: {self.frecuencies_intraday_in_api}', ValueError)

    def check_frecuency_in_key_data(self, key_data, frecuency):
